...
<?php
    ...
    if(isset($_POST['signup'])){
        ...
        if(empty_coor_signup($email, $fname, $phone, $password, $confirm )!== false){
            
            
            header("location: planner_auth.php?error=emptyfield");
            exit();
 
        }

        if(invalid_email($email)!== false){
            header("location: planner_auth.php?error=invalidemail");
        //     echo '<div class="message" id="message">
        //     error: INVALID EMAIL
        // </div>';
        exit();
        ...
        if (invalid_password($password)) {
            header("location: planner_auth.php?error=invalid_pass");
            exit();
        ...
        if(pwd_match($password, $confirm)!== false){
            ...
            header("location: planner_auth.php?error=pwd_not_match");
            exit();
        }

        if(planner_email_exists($conn, $email)!== false){
            header("location: planner_auth.php?error=email_in_use");
        ...
        create_planner($conn, $email, $fname,  $phone, $password, $confirm );
...
<?php
    ...
    if(emptylogin($email, $password)){
        header("location: planner_auth.php?error=empty_login");
        exit();
    }
